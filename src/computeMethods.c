...
void printFibonacci(const uint64_t n, const char radix, const char output, const char *filename, const bool infoInOutputFile) {
    ...
    const size_t estimatedSizeInBytes = (size_t) (0.0868 * (double) n + 3.8275);
    const double sizeInMBEst = ((double) estimatedSizeInBytes) / 1000000;
    ...
    getCurrentTime(&end);
    const double timeToCalc = calcTimeDiff(&start, &end);

    const size_t sizeInBytes = (res->end - res->start) * 8;
    ...
    if (output == 't' || output == 'f') {
        ...
        getCurrentTime(&end2);
        const double timeToOutput = calcTimeDiff(&start2, &end2);

        const double sizeInKB = ((double) sizeInBytes) / 1000;
        const double sizeInMB = ((double) sizeInBytes) / 1000000;

        const double sizeStrInKB = ((double) strSizeInBytes) / 1000;
        const double sizeStrInMB = ((double) strSizeInBytes) / 1000000;
...
void printSquareRoot(const uint64_t n, const uint64_t digits, const char radix, const char output, const char *filename,
                     const bool infoInOutputFile) {
    size_t binaryDigits;
    char *radixStr;
    if (radix == 'd') {
        binaryDigits = ceil((double) digits * log2(10));
    ...
    binaryDigits += 64;
    const size_t wantedFractionBlocks = binaryDigits / 64 + 1;
...
void conversionAndPrintHelper(bigFrac *res, const char *computeName, const char *radixStr, const double timeToCalc, const uint64_t digits,
                              const char radix, const char output, const char *filename, const bool infoInOutputFile) {
    const size_t sizeInBytes = (res->bigIntPart->end - res->bigIntPart->start) * 8;
    if (output == 't' || output == 'f') {
        size_t strSizeInBytes;
        char *resString;
        char *localTime = getCurrentDateTime();
        ...
        struct timespec end2;
        getCurrentTime(&end2);
        const double timeToConvert = calcTimeDiff(&start2, &end2);
        ...
        getCurrentTime(&end3);
        const double timeToOutput = calcTimeDiff(&start3, &end3);
